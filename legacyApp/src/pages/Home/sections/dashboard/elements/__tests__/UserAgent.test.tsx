...
describe("pages / Home / sections / dashboard / elements / UserAgent", () => {
  test("should have correct structure", () => {
    const { BrowserInfo, DashboardElement } = setup();

    expect(
      DashboardElement.children[2].children[0].children[4].children[0]
        .children[0]
    ).toEqual(BrowserInfo);
  });
    ...
    describe("Props", () => {
      describe("childrenHeight", () => {
        test("should have calc(100% - 3.6rem - 2.4rem - 2.8rem)", () => {
          ...
          expect(DashboardElement.children[2]).toHaveStyleRule(
            "height",
      ...
      describe("description", () => {
        test("should have Mozilla/5.0 (win32) AppleWebKit/537.36 (KHTML, like Gecko) jsdom/11.12.0", () => {
          ...
          expect(DashboardElementDescriptionText.textContent).toEqual(
            "Mozilla/5.0 (win32) AppleWebKit/537.36 (KHTML, like Gecko) jsdom/11.12.0"
          );
        });
      });
      ...
      describe("shouldDisplayCorners", () => {
        test("should render corners", () => {
          ...
          expect(DashboardElement.children[2].children[0].children[0]).toEqual(
            Corners[0]
          );
          expect(DashboardElement.children[2].children[0].children[1]).toEqual(
            Corners[1]
          );
          expect(DashboardElement.children[2].children[0].children[2]).toEqual(
            Corners[2]
          );
          expect(DashboardElement.children[2].children[0].children[3]).toEqual(
            Corners[3]
          );
        });
      });
  ...
  describe("BrowserInfo", () => {
    test("should render correct browser info", () => {
      ...
      expect(BrowserInfo.children[0].children[0]).toHaveStyleRule(
        "align-items",
        "center"
      );
      expect(BrowserInfo.children[0].children[0]).toHaveStyleRule(
      ...
      expect(
        BrowserInfo.children[0].children[0].children[0].children[0].textContent
      ).toEqual("Icon-Chrome.svg");
      expect(
        BrowserInfo.children[0].children[0].children[1].children[0].textContent
      ).toEqual("Icon-Firefox.svg");
      expect(
        BrowserInfo.children[0].children[0].children[2].children[0].textContent
      ).toEqual("Icon-IE.svg");
      expect(
        BrowserInfo.children[0].children[0].children[3].children[0].textContent
      ).toEqual("Icon-Opera.svg");
      expect(
        BrowserInfo.children[0].children[0].children[4].children[0].textContent
      ).toEqual("Icon-Safari.svg");
      expect(
        BrowserInfo.children[0].children[0].children[5].children[0].textContent
      ).toEqual("Icon-Unknown.svg");
    });
  });
});
...
function setup(): Setup {
  const utils: RenderResult = renderWithTheme(<UserAgent />);
  ...
  const DashboardElementDescriptionText: Element = queryByTestId(
