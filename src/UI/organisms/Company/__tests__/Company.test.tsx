...
describe("organisms / Company", () => {
  test("should have correct structure for tv, desktop and tablet", () => {
    const {
      CompanyContainer,
      CompanyDescription,
      CompanyInnerFlexContainerTvDesktopAndTablet,
    ...
    expect(CompanyPositionContainer.children[0]).toEqual(ResponsiveTv);
    expect(CompanyPositionContainer.children[1]).toEqual(ResponsiveDesktop);
    expect(CompanyPositionContainer.children[2]).toEqual(ResponsiveTablet);

    expect(ResponsiveTv.children[0]).toEqual(TimelinePositionContainer);
    expect(ResponsiveDesktop.children[0]).toEqual(TimelinePositionContainer);
    expect(ResponsiveTablet.children[0]).toEqual(TimelinePositionContainer);

    expect(TimelinePositionContainer.children[0]).toEqual(Timeline);

    CompanyInnerFlexContainerTvDesktopAndTablet.forEach((_CompanyInnerFlexContainerTvDesktopAndTablet: Element) => {
      expect(_CompanyInnerFlexContainerTvDesktopAndTablet.children[0]).toEqual(FlexItem[0]);
      expect(_CompanyInnerFlexContainerTvDesktopAndTablet.children[1]).toEqual(FlexItem[1]);
    });
  ...
  test("should have correct structure for mobile", () => {
    const {
      CompanyContainer,
      CompanyDescription,
      CompanyInnerFlexContainerMobile,
      CompanyPositionContainer,
      Logo,
      LogoFlexContainer,
      ResponsiveMobile
    ...
    expect(CompanyPositionContainer.children[3]).toEqual(ResponsiveMobile);
    expect(ResponsiveMobile.children[0]).toEqual(CompanyInnerFlexContainerMobile);

    expect(CompanyInnerFlexContainerMobile.children[0]).toEqual(LogoFlexContainer);
    expect(LogoFlexContainer.children[0]).toEqual(Logo);
    
    expect(CompanyInnerFlexContainerMobile.children[1]).toEqual(CompanyDescription);
  });
...
interface Setup extends RenderResult {
  ...
  CompanyInnerFlexContainerMobile: Element;
  CompanyInnerFlexContainerTvDesktopAndTablet: Element[];
...
function setup(additionalProps?: CompanyTestProps): Setup {
  ...
  const CompanyInnerFlexContainerMobile: Element = queryAllByTestId("CompanyInnerFlexContainerMobile")[0];
  const CompanyInnerFlexContainerTvDesktopAndTablet: Element[] = queryAllByTestId("CompanyInnerFlexContainerTvDesktopAndTablet");
