...
describe("pages / Home / sections / experience / companies / CompanyPersonallyEmployed", () => {
  describe("Props", () => {
    ...
    describe("date", () => {
      test("should render May 2008 to September 2010", () => {
        ...
        CompanyDescriptions.forEach((CompanyDescription: Element) => {
          ...
          ).toEqual("May 2008 to September 2010");
        });
      });
    });
    ...
    describe("timelineBottom", () => {
      test("should have 0", () => {
        const { TimelinePositionContainers } = setup();

        TimelinePositionContainers.forEach(
...
function setup(): Setup {
  const utils: RenderResult = renderWithTheme(<CompanyPersonallyEmployed />);

  const { queryAllByTestId } = utils || {};

  const CompanyDescriptions: Element[] = queryAllByTestId("CompanyDescription");
  const Logos: Element[] = queryAllByTestId("Logo");
  const ResponsibilitiesSpacingContainers: Element[] = queryAllByTestId(
    "ResponsibilitiesSpacingContainer"
  );
  const TimelinePositionContainers: Element[] = queryAllByTestId(
